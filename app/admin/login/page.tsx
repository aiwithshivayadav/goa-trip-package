 ...
 * Left: branded illustration / Right: login form
 */
      ...
      <div className="flex w-full lg:w-1/2 items-center justify-center bg-cosmic-950 px-6">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="gtpadmin"
                required
                className="w-full h-12 rounded-lg bg-surface border border-border-gold px-4 text-white placeholder:text-text-dim focus:outline-none focus:border-gold focus:ring-1 focus:ring-gold transition-colors"
              />
            </div>
            ...
            {error && (
              <p className="text-sm text-rose animate-[shake_0.3s_ease-in-out]">
                {error}
              </p>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 rounded-lg bg-gold-gradient text-cosmic-950 font-bold text-sm transition-transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <p className="mt-8 text-center text-sm text-text-dim">
            Need help?{" "}
            <a
              href={`https://wa.me/919890830249?text=${encodeURIComponent("Admin login help needed")}`}
