    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-hero-gradient py-20 text-center md:py-28">
        <div className="relative z-10 mx-auto max-w-3xl px-6">
          <p className="text-sm uppercase tracking-[0.15em] text-amber mb-3">Our Story</p>
          <h1 className="font-display text-4xl font-bold text-white md:text-5xl">
            About <span className="text-lagoon-100">Goa Trip Package</span>
          </h1>
      ...
      <section className="border-y border-border-warm bg-ground py-8">
        <div className="mx-auto max-w-5xl px-4 md:px-8">
          <div className="grid grid-cols-2 gap-6 md:grid-cols-4 text-center">
            <div><p className="text-2xl font-bold text-ink">10,000+</p><p className="text-xs text-gray-500 mt-0.5">Happy Travellers</p></div>
            <div><p className="text-2xl font-bold text-ink">4.8★</p><p className="text-xs text-gray-500 mt-0.5">Average Rating</p></div>
            <div><p className="text-2xl font-bold text-ink">50+</p><p className="text-xs text-gray-500 mt-0.5">Curated Experiences</p></div>
            <div><p className="text-2xl font-bold text-ink">9 Years</p><p className="text-xs text-gray-500 mt-0.5">Operating in Goa</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-4xl px-4 py-16 md:px-8">
        <div className="rounded-2xl border border-border-warm bg-white p-8 md:p-12">
          <h2 className="font-display text-2xl font-bold text-ink mb-6">
            From a WhatsApp Number to Goa&apos;s <span className="text-lagoon">Largest Trip Platform</span>
          </h2>
      ...
      {/* Timeline */}
      <section className="py-16 bg-ground border-y border-border-warm">
        <div className="mx-auto max-w-3xl px-4 md:px-8">
          <h2 className="font-display text-2xl font-bold text-ink text-center mb-12">Our Journey</h2>
          <div className="relative">
            <div className="absolute left-4 top-0 bottom-0 w-px bg-border-warm md:left-1/2" />
            {milestones.map((m, i) => (
              <div key={m.year} className={`relative flex gap-6 mb-8 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
                <div className="w-8 shrink-0 flex flex-col items-center md:w-1/2 md:items-end md:pr-8">
                  {i % 2 === 0 && <div className="hidden md:block text-right"><p className="text-sm font-bold text-lagoon">{m.year}</p><p className="text-sm font-medium text-ink mt-1">{m.title}</p><p className="text-xs text-gray-500 mt-1">{m.desc}</p></div>}
                  {i % 2 !== 0 && <div className="hidden md:block text-left"><p className="text-sm font-bold text-lagoon">{m.year}</p><p className="text-sm font-medium text-ink mt-1">{m.title}</p><p className="text-xs text-gray-500 mt-1">{m.desc}</p></div>}
                </div>
                <div className="absolute left-2 md:left-1/2 md:-translate-x-1/2 h-5 w-5 rounded-full bg-lagoon-50 border-2 border-lagoon flex items-center justify-center">
                  <div className="h-2 w-2 rounded-full bg-lagoon" />
                </div>
                <div className="flex-1 pl-8 md:w-1/2 md:pl-8 md:hidden">
                  <p className="text-sm font-bold text-lagoon">{m.year}</p>
                  <p className="text-sm font-medium text-ink mt-1">{m.title}</p>
      ...
      <section className="mx-auto max-w-5xl px-4 py-16 md:px-8">
        <h2 className="font-display text-2xl font-bold text-ink text-center mb-10">What We Stand For</h2>
        <div className="grid gap-6 sm:grid-cols-2">
          {values.map((v) => {
            const Icon = v.icon;
            return (
              <div key={v.title} className="rounded-xl border border-border-warm bg-white p-6">
                <Icon className="h-6 w-6 text-lagoon mb-3" />
      ...
      {/* CTA */}
      <section className="border-t border-border-warm bg-ground py-16">
        <div className="mx-auto max-w-2xl px-6 text-center">
          <h2 className="font-display text-2xl font-bold text-ink">Ready to Plan Your Goa Trip?</h2>
          <p className="mt-3 text-sm text-gray-500">Our team responds within 30 minutes on WhatsApp.</p>
          <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/custom-trip" className="inline-flex h-11 items-center justify-center rounded-full bg-lagoon px-6 text-sm font-bold text-white">
              Get Free Quote
            </Link>
            <a href="https://wa.me/919890830249" className="inline-flex h-11 items-center justify-center rounded-full border border-border-warm px-6 text-sm text-lagoon">
              WhatsApp Us
            </a>
          </div>
        </div>
