...
import { activities, type ProductData } from "@/lib/data/products";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const p = activities.find((a) => a.slug === slug);
  if (!p) return { title: "Activity Not Found" };
...
export default async function ActivityDetailPage({ params }: PageProps) {
  ...
  const similarProducts = activities.filter((a) => a.slug !== slug).slice(0, 3);
  ...
  return (
    <div className="min-h-screen pb-20 lg:pb-0">
      ...
      <div className="mx-auto max-w-7xl px-4 pt-4 md:px-8">
        <Link href="/activities" className="inline-flex items-center gap-2 text-xs text-gray-500 hover:text-lagoon transition-colors">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="lg:hidden">
              <div className="flex items-center gap-2 mb-2">
                <span className="rounded-md bg-lagoon-50 px-2 py-0.5 text-[10px] font-bold text-amber uppercase tracking-wider">Adventure Activity</span>
            ...
            <div className="flex flex-wrap gap-3">
              <span className="flex items-center gap-1.5 rounded-lg bg-white border border-border-warm px-3 py-2 text-xs text-gray-500">
            ...
            {p.itinerary && p.itinerary.length > 0 && (
              <div>
                ...
                <div className="space-y-3">
                  {p.itinerary.map((day) => (
                    <div key={day.day} className="rounded-xl border border-border-warm bg-white p-5">
                      <div className="flex items-start gap-4">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-lagoon-50 text-sm font-bold text-lagoon">D{day.day}</div>
                        <div>
                          <h3 className="font-bold text-ink">{day.title}</h3>
              ...
              <div className="rounded-xl border border-border-warm bg-white p-5">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {p.whatToBring.map((item) => (
                    <span key={item} className="rounded-full bg-white border border-border-warm px-3 py-1 text-xs text-gray-500">{item}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {p.faq && p.faq.length > 0 && (
              <div>
                <h2 className="text-lg font-bold text-ink mb-4 flex items-center gap-2">
                  <HelpCircle className="h-5 w-5 text-lagoon" /> Frequently Asked Questions
                </h2>
                <div className="space-y-2">
                  {p.faq.map((item) => (
                    <details key={item.q} className="rounded-xl border border-border-warm bg-white group">
                      <summary className="flex items-center justify-between px-5 py-4 text-sm font-medium text-ink cursor-pointer list-none">
                        {item.q}
            ...
            {similarProducts.length > 0 && (
              <div>
                <h2 className="text-lg font-bold text-ink mb-4">Similar Experiences</h2>
          ...
          <div className="hidden lg:block">
            <div className="sticky top-24 space-y-4">
              ...
              <div className="rounded-2xl border border-border-warm bg-white p-6">
                <div className="text-center mb-4">
                  ...
                  <div className="mt-1 flex items-baseline justify-center gap-2">
                    {p.originalPrice && (
                      <span className="text-base text-gray-400 line-through">{formatINR(p.originalPrice)}</span>
                    )}
                ...
                <div className="space-y-2.5 text-sm mb-5">
                  {p.duration && (
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1.5 text-gray-500"><Clock className="h-3.5 w-3.5" /> Duration</span>
                      <span className="text-ink font-medium">{p.duration.split("(")[0]?.trim()}</span>
                    </div>
                  )}
                  {p.capacity && (
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1.5 text-gray-500"><Users className="h-3.5 w-3.5" /> Capacity</span>
                      <span className="text-ink font-medium">{p.capacity}</span>
                    </div>
                  )}
                  {p.location && (
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1.5 text-gray-500"><MapPin className="h-3.5 w-3.5" /> Location</span>
                      <span className="text-ink font-medium text-right max-w-[50%] text-xs">{p.location}</span>
                    </div>
                  )}
                  {p.timing && (
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1.5 text-gray-500"><Calendar className="h-3.5 w-3.5" /> Timing</span>
                      <span className="text-ink font-medium text-xs">{p.timing.split("(")[0]?.trim()}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  {isSelfServe ? (
                    <Link
                      href={`/checkout?product=${p.slug}&type=activity`}
                      className="flex h-12 w-full items-center justify-center rounded-xl bg-lagoon text-sm font-bold text-white transition-transform hover:scale-[1.02] active:scale-[0.98]"
                    >
                      Book Now — {formatINR(p.basePrice)}
                    </Link>
                  ) : (
                    <EnquiryButton productName={p.name} productSlug={p.slug} productType="activity" productPrice={p.basePrice} variant="gold" label="Get Custom Quote" />
                  )}
                  <EnquiryButton productName={p.name} productSlug={p.slug} productType="activity" productPrice={p.basePrice} label="WhatsApp Enquiry" />
                </div>
              ...
              {/* Social proof */}
              <div className="rounded-xl border border-border-warm bg-white p-4 text-center">
                <div className="flex items-center justify-center gap-1.5 text-xs text-gray-500">
                  <Zap className="h-3.5 w-3.5 text-lagoon fill-amber" />
                  <span><strong className="text-ink">12 people</strong> booked in the last 24 hours</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-border-warm bg-white/95 backdrop-blur-lg p-4 lg:hidden">
        <div className="flex items-center gap-3">
          ...
          {isSelfServe ? (
            <Link href={`/checkout?product=${p.slug}&type=activity`} className="flex h-11 items-center justify-center rounded-xl bg-lagoon px-6 text-sm font-bold text-white">Book Now</Link>
          ) : (
            <EnquiryButton productName={p.name} productSlug={p.slug} productType="activity" productPrice={p.basePrice} variant="outline" label="Get Quote" />
          )}
          <EnquiryButton productName={p.name} productSlug={p.slug} productType="activity" productPrice={p.basePrice} variant="compact" />
        </div>
      </div>
    </div>
  );
