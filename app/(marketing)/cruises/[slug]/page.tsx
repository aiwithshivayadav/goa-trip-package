...
import { cruises, type ProductData } from "@/lib/data/products";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const p = cruises.find((c) => c.slug === slug);
...
export default async function CruiseDetailPage({ params }: PageProps) {
  ...
  const similarProducts = cruises.filter((c) => c.slug !== slug).slice(0, 3);
  ...
  return (
    <div className="min-h-screen pb-20 lg:pb-0">
      ...
      <div className="mx-auto max-w-7xl px-4 pt-4 md:px-8">
        <Link href="/cruises" className="inline-flex items-center gap-2 text-xs text-gray-500 hover:text-lagoon transition-colors">
          <ArrowLeft className="h-3.5 w-3.5" /> All Cruises
        </Link>
      </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="lg:hidden">
              <div className="flex items-center gap-2 mb-2">
                <span className="rounded-md bg-lagoon-50 px-2 py-0.5 text-[10px] font-bold text-amber uppercase tracking-wider">Cruise</span>
            ...
            <div className="sticky top-24 space-y-4">
              ...
              <div className="rounded-2xl border border-border-warm bg-white p-6">
                ...
                <div className="space-y-3">
                  {isSelfServe ? (
                    <Link
                      href={`/checkout?product=${p.slug}&type=cruise`}
                    ...
                    <EnquiryButton productName={p.name} productSlug={p.slug} productType="cruise" productPrice={p.basePrice} variant="gold" label="Get Custom Quote" />
                  )}
                  <EnquiryButton productName={p.name} productSlug={p.slug} productType="cruise" productPrice={p.basePrice} label="WhatsApp Enquiry" />
                </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-border-warm bg-white/95 backdrop-blur-lg p-4 lg:hidden">
        <div className="flex items-center gap-3">
          ...
          {isSelfServe ? (
            <Link href={`/checkout?product=${p.slug}&type=cruise`} className="flex h-11 items-center justify-center rounded-xl bg-lagoon px-6 text-sm font-bold text-white">Book Now</Link>
          ) : (
            <EnquiryButton productName={p.name} productSlug={p.slug} productType="cruise" productPrice={p.basePrice} variant="outline" label="Get Quote" />
          )}
          <EnquiryButton productName={p.name} productSlug={p.slug} productType="cruise" productPrice={p.basePrice} variant="compact" />
        </div>
      </div>
    </div>
  );
